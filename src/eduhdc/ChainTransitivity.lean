...
/-- Functoriality of chain composition: concatenating two relation lists and
    running the roundtrip is the same as running the second list's roundtrip
    first, then the first list's roundtrip on the result. Trivially true because
    `chainRoundtrip P l Y = Y` for every list `l` (by `chain_exact_unbind`). -/
theorem chainRoundtrip_append {V : Type} (P : PedagogicalVSA V)
    (l1 l2 : List Nat) (Y : V) :
    chainRoundtrip P (l1 ++ l2) Y = chainRoundtrip P l1 (chainRoundtrip P l2 Y) := by
  rw [chain_exact_unbind P (l1 ++ l2) Y,
      chain_exact_unbind P l2 Y,
      chain_exact_unbind P l1 Y]
