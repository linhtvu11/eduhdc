...
def run_v4():
    ...
    # the test set is always the test set (no adaptive retreat).
    ...
    #  (3) TRANSITIVE CLOSURE: if (a,b) and (b,c) positive, add (a,c) > (c,a).
    #      Valid inductive bias: prerequisite is transitive. Train-split only.
    pos_set = set(tr_pos)
    out_nei = {}
    for (a, b) in tr_pos:
        out_nei.setdefault(a, []).append(b)
    trans_fwd, trans_rev = [], []
    for (a, b) in tr_pos:
        for c in out_nei.get(b, []):
            if (a, c) not in pos_set and a != c:
                trans_fwd.append((a, c)); trans_rev.append((c, a))
    # Combine
    all_fwd = fwd_tr + uni_fwd + trans_fwd
    all_rev = rev_tr + uni_rev + trans_rev
    Xu_f_tr, Xv_f_tr = build_tensors(all_fwd, ex_to_dense, device)
    Xu_r_tr, Xv_r_tr = build_tensors(all_rev, ex_to_dense, device)
    print(f"Directional TRAIN constraints: bidir={len(fwd_tr)} + unidir={len(uni_fwd)} "
          f"+ trans={len(trans_fwd)} = {len(all_fwd)}")
    ...
    try:
        ...
        with open(os.path.join(RESULTS_DIR, "prereq_probing_v5_results.json"), "w") as f:
            json.dump(all_results, f, indent=2)
        print(f"[saved: {RESULTS_DIR}\\prereq_probing_v5_results.json]")
