...
class EduBindBlockDiag(BaseVSA):
    ...
    def random_vector(self, n: int = 1, generator: Optional[torch.Generator] = None) -> torch.Tensor:
        """Fast analytical O(2) random orthogonal matrix generation on GPU.

        `generator` is optional and defaults to None (uses the global RNG,
        unchanged behaviour for every existing caller). Pass an explicit
        `torch.Generator` to make the sampled vector depend only on that
        generator's seed rather than on global RNG call order."""
        theta = torch.rand(n, self.num_blocks, device=self.device, generator=generator) * 2.0 * torch.pi
        s = torch.randint(0, 2, (n, self.num_blocks), device=self.device, generator=generator).float() * 2.0 - 1.0
...
class EduBindComplexUnitary(BaseVSA):
    ...
    Partitions complex Hilbert space into 2x2 U(2) unitary blocks.
    Matrix multiplication in U(2) is non-commutative and preserves L2 energy exactly.

    Note on the group: `random_vector` draws four angles, so each block has
    determinant exp(i*chi) and the family is U(2), not SU(2) as an earlier
    version of this docstring said. The Lean side proves the unitary case
    (`EduHDC.unitaryVSA`, src/eduhdc_mathlib/GroupActionReal.lean), which is what
    this sampler actually produces and is the more general statement.
    """
    def __init__(self, dim: int = 10000, device: Optional[str] = None):
        super().__init__(dim=dim, device=device)
        self.block_size = 2
