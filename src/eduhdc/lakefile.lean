...
lean_lib «EduHDC» where
  ...
  globs := #[.one `EduBindSelfContained, .one `VSATriad, .one `ChainTransitivity,
             .one `CapacityCostModel, .one `SpecStrengthening, .one `EncPairSpec,
             .one `ChainCrosstalk, .one `GroupActionSpec, .one `MonoidRelaxation,
             .one `ChainOrder, .one `DihedralLabel]
